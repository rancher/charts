...
{{/* Create a default name for the custome template confirmap */}}
{{- define "nginx-ingress.controller.template.configmap" -}}
{{- printf "%s-template" (include "nginx-ingress.controller.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{- define "dashboard.ingress.hosts.0" -}}
{{- $certName := .Values.dashboard.certificate | toString -}}
{{- $envName := .Values.dashboard.environment | toString -}}
{{- if .Values.dashboard.addEnvironmentToDNSNames -}}
  {{- printf "%s-dashboard.%s" $envName $certName -}}
{{- else -}}
  {{- printf "dashboard.%s" $certName -}}
{{- end -}}
{{- end -}}

{{- define "dashboard.ingress.tls.0" -}}
{{- $certName := .Values.dashboard.certificate | toString -}}
